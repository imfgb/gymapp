...
- **nutrition-meal-variety** (2026-05-24): the ~17 hand-written `MEAL_TEMPLATES` are gone — they made generation feel repetitive (a given preference set matched only 1–2 breakfast templates, so the user "always" got avena+whey+plátano+crema). `MEAL_TEMPLATES` is now **generated combinatorially** by `_build_meal_templates()` from per-style building-block pools (sweet, egg, savory, light-snack) → **~2066 coherent templates** (188 breakfast / 1872 lunch+dinner / 146 snack). Coherence is preserved by construction: sweet fats (peanut/almond butter, dark chocolate) only appear in sweet meals, eggs only in egg breakfasts, savory meats only in savory plates — so no "steak + peanut butter". `generate_meal` now also **rotates the vegetable** through the user's liked vegetables (`_apply_veg_preference`): respects the preference *and* adds variety (a meal still only contains liked items when the user has set veg prefs). Adding a food to a pool multiplies variety. Template `name` is auto-derived (`_meal_name`) and only used for admin/debug — the UI renders per-food items, not the name. Tests assert: library size + per-slot counts, no sweet fats on savory plates, no savory meats in sweet meals, breakfast variety with rich prefs (>8 distinct in 30 gens), and veg rotation stays within liked items.
