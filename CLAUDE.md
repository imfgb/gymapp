...
- **`for_user` does NOT bypass for superusers.** A superuser is just another user in the regular app; they only see their own rows on dashboard/routines/metrics/PRs. Superuser-sees-all lives in `/admin` only, via `OwnerScopedAdmin.get_queryset` (which has its own short-circuit). A past bypass in `for_user` leaked every user's data into the superuser's regular pages — see `tests/apps/core/test_owner_scoping.py`.
- **Cross-owner FK leaks.** When you follow a FK from an owner-scoped row to another owner-scoped row (e.g. `WeeklySplit.routine_day → RoutineDay → Routine.owner`), defense-in-depth re-checks `routine.owner_id == request.user.id` in the render path so legacy/stale rows can't leak the other user's data.
...
- **mobile responsive overhaul** (2026-05-25): a second pass at 390px (iPhone 13/Safari) browser-verified with Playwright + WebKit. (1) **Workout exercise card**: header now stacks the name above the action buttons on mobile (`flex flex-col gap-2 sm:flex-row sm:items-baseline sm:justify-between`), name wraps with `min-w-0 break-words`, buttons wrap (`flex flex-wrap`). (2) **Routine day-card editor**: the per-exercise row used to crush the name to "D…" next to 5 inline number inputs; restructured to a `flex justify-between` header (name + ✕) plus a `grid grid-cols-2 sm:grid-cols-5` for Series/Reps min/Reps max/Peso/Descanso with `w-full` inputs. (3) **Tables** (`metrics/list`, `prs/list`, `prs/detail`): wrapped in `overflow-x-auto` with `whitespace-nowrap` on the date/numeric cells so rows stay one-line and scroll horizontally inside their own container. (4) **Pickers/swap options**: `truncate` → `break-words` everywhere — exercise names never get cut. (5) **Nutrition macros**: "G CARBOHIDRATOS" was clipping in its narrow card; `text-[11px] leading-tight` + `p-3 sm:p-4` so the full word fits. Always `npm run build:css` after Tailwind class changes — the deployed prod CSS must include the new arbitrary values (`text-[11px]`, `sm:grid-cols-5`).

- **scheduling: auto-program the week** (2026-05-25): generating a routine never wrote `WeeklySplit`, so the dashboard "Hoy"/"Esta semana" stayed empty and the "Hoy no iré al gym" button was hidden (it only renders when `today_routine_day` is truthy). Added `WEEKDAY_PATTERNS` + `assign_weekly_split(owner, routine)` in `services/routine_generator/__init__.py` (1 day → Mon; 2 → Mon/Thu; 3 → Mon/Wed/Fri; 4 → Mon/Tue/Thu/Fri; 5 → Mon-Fri; 6 → Mon-Sat; 7 → all). Wired into `routine_create` so the generator auto-schedules. New view + button **"Programar en la semana"** on routine detail (`routines:apply_to_week`) lets any routine (incl. hand-built) be spread across the week with one click. The split editor (`templates/routines/weekly_split.html`) was 7 per-day forms with 7 separate "Guardar" buttons — changing several rows and pressing one Guardar discarded the rest. Replaced with a **single bulk form** (one `Guardar semana` + a `Llenado rápido` row that calls `apply_to_week`) backed by `weekly_split_save` (kept the per-day `weekly_split_assign` for compat). Parametrized tests cover every WEEKDAY_PATTERN size.

- **CRITICAL privacy fix — superuser bypass in `for_user`** (2026-05-26): `OwnerScopedQuerySet.for_user()` short-circuited to `self.all()` when `user.is_superuser` was true. The superuser (`fglzb00@gmail.com`) therefore saw every user's data on the regular pages (dashboard, routines, metrics, PRs, weekly split). Surfaced when a second account was created and the superuser saw that user's new routine + metrics on their own home. The bypass is removed; `/admin` is unaffected because `OwnerScopedAdmin.get_queryset` has its own superuser branch and doesn't call `for_user`. Also added defense-in-depth in dashboard + workouts.start: when following `WeeklySplit.routine_day → Routine.owner`, re-check ownership so legacy split rows from before the fix can't leak another user's routine label. Regression suite in `tests/apps/core/test_owner_scoping.py` covers queryset behaviour AND the rendered dashboard/routines/metrics pages. See memory `[[project-superuser-privacy]]`.
