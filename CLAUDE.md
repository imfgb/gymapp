...
- **nutrition-meals-plus** (2026-05-23): curated `FOOD_CATALOG` (whey concentrate/isolate, casein, salmon, lean/ground beef, …; user trimmed it) + `FOOD_MACROS` table (protein/carbs/fat per 100 g, raw/dry). `services.nutrition.generate_meal(slot, target, prefs, rng)` picks a food per slot category (random → variety, unlike the deterministic `build_meal_plan`) and **sizes each in raw grams** so its namesake macro hits the slot's share of the daily target (protein source→protein, carb→carbs, fat→fat; veg = fixed serving), then sums the real per-food macros → `GeneratedMeal(items: list[MealItem(slug, grams, p/c/f, kcal)], totals)`. So grams ↔ macros are consistent and self-explanatory (the meal total can exceed the slot target when a fatty protein/cross-macros add up — that's the honest reality). `nutrition.SavedMeal` stores the items (with grams) + totals + `eaten_at`. Views: generate (`nutrition:generate_meal`), toggle eaten w/ timestamp (`nutrition:meal_done`), delete (`nutrition:meal_delete`). The nutrition page shows per-food grams + macro breakdown ("Mis comidas") + a "Generar comida" control.
