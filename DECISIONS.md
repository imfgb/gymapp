...
- **ADR-006: Metric units only (kg, cm).** No conversion layer. (2026-05-19, discovery #6) — *Amended by ADR-027: lifted weight may display in kg or lb per exercise (still stored in kg). Bodyweight + heights remain metric.*
...
- **ADR-027: Exercise weight is kg-canonical with per-exercise kg/lb display (amends ADR-006).** Lifted weight is always stored in kg (`SetLog`/`RoutineExercise`/`PersonalRecord`), so tonnage and PRs aggregate without unit ambiguity. `Exercise.weight_unit` (blank = auto: **lb for cable, kg otherwise**; machines default kg because plate-loaded ones — hack squat, leg press — use kg discs; flipped per exercise via a kg⇄lb toggle, no lock since storage is kg) governs only input and display. `gymapp/services/units.py` converts at the view boundary (display→kg on save) and a template filter (`in_unit`/`weight_unit_label`) at render; progression advances lb exercises by +5 lb and warm-ups snap to 5-lb steps. Bodyweight (`UserMetricSnapshot`, `MonthlyGoal`) and heights stay metric. (2026-06-08, feedback #8) See `docs/superpowers/specs/2026-06-08-exercise-weight-units-design.md`.
